...
cur.execute('SELECT `id`,`picture_1_location` FROM `card` WHERE `processed_file_location` IS NULL AND `category`="MSH Kenya"')
...
for row in cur.fetchall() :
    ...
    #run lanes.py
    inp = ["python", "lanes.py", "-r", "processed", "-t", "lanetemplate.png", filename]
    p = subprocess.Popen(inp, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    (stdout, stderr) = p.communicate()
    code = p.returncode
    ...
    if code==0:
        ...
        f = open("processed_db.txt", "a+")
        f.write(filename + '\n')
        f.close()
    else:
        #write error file to file
        f = open("not_processed_db.txt", "a+")
        f.write(filename + '\n')
        f.close()
